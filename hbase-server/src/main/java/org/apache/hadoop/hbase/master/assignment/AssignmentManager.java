...
public class AssignmentManager {
  ...
  public void start() throws IOException, KeeperException {
    ...
    List<String> rootZNodes = zkw.getRootReplicaNodes();
    LOG.debug("hbase:root replica znodes: {}", rootZNodes);
    for (String rootZNode : rootZNodes) {
      int replicaId = zkw.getZNodePaths().getMetaReplicaIdFromZNode(rootZNode);
      ...
      // region node.
      RegionState rootRegionState = RootTableLocator.getRootRegionState(zkw, replicaId);
      RegionStateNode rootRegionNode =
        regionStates.getOrCreateRegionStateNode(rootRegionState.getRegion());
      ...
      LOG.debug("Loaded hbase:root {}", rootRegionNode);
    }
  }
